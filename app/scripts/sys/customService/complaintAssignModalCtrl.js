customServiceApp.controller('ComplaintAssignModalCtrl', ['$scope', 'close', 'MendSecurityComplaintService', 'title', 'initVal','$timeout','$interval', 'udcModal',function ($scope, close, MendSecurityComplaintService, title, initVal,$timeout,$interval,udcModal) {
    $scope.dataSource = {};
    $scope.chart = null;
    ...
    $scope.submit = function () {
        if($scope.vm.selectDepartment.code!=null){
            $scope.vm.complaint.department = $scope.vm.selectDepartment;
            $scope.vm.complaint.processStatus = "PTHandling";
            MendSecurityComplaintService.modifyComplaint($scope.vm.complaint).then(function () {
